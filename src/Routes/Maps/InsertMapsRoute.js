...
const filePath = path.join("Public", "Maps").replace(/\\/g, '/');
const {InsertMapModel} = require('../../MongoDB/Modals/Maps.Model')
...
async function InsertMapsRoute(req,resp){
    ...
    try {
      ...
      upload.any()(req, resp, function (err) {
        ...
        const modifiedFiles = files.map(file => ({
          ...file,
          path: path.join('Public/Maps/Gallery', path.basename(file.path)).replace(/\\/g, '/')
        }));
...
module.exports={InsertMapsRoute}